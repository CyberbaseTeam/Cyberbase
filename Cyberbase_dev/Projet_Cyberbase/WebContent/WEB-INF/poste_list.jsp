									...
									<li>- ${poste.nom_poste} : 
										...
										<c:choose>
											<c:when test="${poste.disponibilite}">
												...
												<a href="affecter_poste?id=${poste.id_poste}">Affecter poste</a> 
											</c:when>
											<c:otherwise>
												<span>Poste indisponible</span> 
													<input type="hidden" value="${poste.id_poste}" name="inputIdPoste"/>
													<input type="submit" value="Libérer poste" name="libererPoste">
											</c:otherwise>
										</c:choose>
									</li>
								  </c:forEach>
